...
function Proccess_Har_Data(){
    ...
    */
    ...
    //}else {
        ...
        const fileName = document.getElementById('getHar').files[0].name;

        fetch('../har_files/' + fileName)
        ...
        function proccess(har) {
            ...
            for (let i =0; i < har.length; i++){
                ...
                for (let j =0; j< har[i].request.headers.length; j++){
                    let requestHeader = {};
                    let flag=0;
        
                    if (har[i].request.headers[j].name =="content-type"){
                        requestHeader = {"name": har[i].request.headers[j].name , "value": har[i].request.headers[j].value };
                        flag = 1;
                ...
                for ( let j=0; j< har[i].response.headers.length; j++){
                    let responseHeader = {};
                    let flag =0;
        
                    if (har[i].response.headers[j].name == "content-type"){
                        responseHeader = {"name": har[i].response.headers[j].name , "value": har[i].response.headers[j].value  };
                        flag = 1;
                    } else if ( har[i].response.headers[j].name =="cache-control"){
                        responseHeader = {"name": har[i].response.headers[j].name , "value": har[i].response.headers[j].value };
                        flag = 1;
                    } else if ( har[i].response.headers[j].name =="pragma"){
                        responseHeader = {"name": har[i].response.headers[j].name , "value": har[i].response.headers[j].value  };
                        flag = 1;
                    } else if ( har[i].response.headers[j].name =="expires"){
                        responseHeader = {"name": har[i].response.headers[j].name , "value": har[i].response.headers[j].value };
                        flag = 1;
                    } else if ( har[i].response.headers[j].name =="age"){
                        responseHeader = {"name": har[i].response.headers[j].name , "value": har[i].response.headers[j].value };
                        flag = 1;
                    } else if ( har[i].response.headers[j].name =="last-modified"){
                        responseHeader = {"name": har[i].response.headers[j].name , "value": har[i].response.headers[j].value };
                        flag = 1;
                    } else if ( har[i].response.headers[j].name =="host"){
                        responseHeader = {"name": har[i].response.headers[j].name , "value": har[i].response.headers[j].value };
                        flag = 1;
                    }
...
function uploadFile(){
    ...
    fetch("https://api.ipify.org/?format=json")
        ...
        .then(data => {
            ...
            fetch('http://ip-api.com/json/' + IPs)
                .then( response => response.json())
                .then( data => {
                    serverUser.push({ "serverIPAddress": data.query, "Isp_name": data.isp, "Latitude": data.lat, "Longitude": data.lon,  
                                            "uploadDateTime": uploadDateTime , "numberOfRecords": numberOfRecords});
    ...
    $.ajax({
        ...
        method: "POST",
        data: { proccessDataReq2 : proccessDataReq2 },
        success: function(res){
            console.log(res);
        }
    });
    ...
    proccessServerIP.forEach( data => {
        serveripaddress = data['entries']['serverIPAddress'];
        fetch('https://freegeoip.app/json/' + serveripaddress)
            .then( response => response.json())
            .then( data => {
                //console.log({"serverIPaddresss": data.ip, "latitude": data.latitude, "longitude": data.longitude });
                dataServer.push({"serverIPaddresss": data.ip, "latitude": data.latitude, "longitude": data.longitude });
                count++;
...
function showHeatMap() {
    ...
    .then ( data => {
        ...
        var cfg = {
            ...
            minOpacity: 0.1,
            maxOpacity: .8,
            gradient: {0.10: 'blue', 0.40: 'lime', 1: 'red'},
            scaleRadius: true,
       ...
       var i=0;
        data.forEach(element => {
            mapData.push( { lat: element['latitude'], lng: element['longitude'], count: element['count'] } );
